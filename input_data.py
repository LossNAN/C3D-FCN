...
def read_clip_and_label(filename, batch_size, start_pos=-1, num_frames_per_clip=16, crop_size=112, shuffle=False):
  ...
  for index in video_indices:
    ...
    if len(line)==2 :
      ...
      if(len(tmp_data)!=0):
        ...
        for i in range(0,num_frames_per_clip):
            for j in range(len(frame.split())):
                if j%2==0:
                    if s_index+i>=int(float(frame.split()[j])*5) and s_index+i<=int(float(frame.split()[j+1])*5)-1:
                        frame_label.append(int(tmp_label))
                        break
                if j==len(frame.split())-1:
  ...
  return np_arr_data, np_arr_label.reshape(batch_size,num_frames_per_clip), next_batch_start, read_dirnames